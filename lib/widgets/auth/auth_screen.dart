...
class AuthScreen extends StatefulWidget {
  ...
  const AuthScreen({super.key, required this.showNotification});
  ...
  void _submitAuthForm() async {
    ...
    try {
      if (_authMode == AuthMode.login) {
        ...
        // 2. Save UserType and Name to Firestore
        Map<String, dynamic> userData = {
          'name': _name,
          'type': _userType.name, // Saves 'farmer' or 'vendor' string
          'email': _email.trim(),
          'createdAt': Timestamp.now(),
        };
        
        // Add vendor-specific fields if user is vendor
        if (_userType == UserType.vendor) {
          userData['licenceNumber'] = _licenceNumber.trim();
          userData['rcNumber'] = _rcNumber.trim();
        }
        
        await _firestore.collection('users').doc(userCredential.user!.uid).set(userData);

        widget.showNotification(
          "Registration successful! Welcome $_name. Logging you in...",
          "success",
        );
        
        // Automatically log out and show login screen
        await Future.delayed(const Duration(seconds: 1));
        // User is already logged in after registration, no need to switch
      }
      ...
      widget.showNotification(
          "An unexpected error occurred: ${e.toString()}", "error");
    } finally {
      if (mounted) {
        setState(() {
          _isLoading = false;
